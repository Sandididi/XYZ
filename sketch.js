...
function setup() {
  ...
  imageMode(CENTER);
  angleMode(DEGREES);
  var canv = createCanvas(windowWidth-10, windowHeight-10, WEBGL);
  frameRate(24);
...
function draw() {
  ...
  push();
    ...
    push();
      ...
      texture(leafTx);
      TorusEmoji(leaf[0],screen,screen/2,360,25,12,0);
      rotateY(turn/3);
      TorusEmoji(leaf[1],screen,screen/2,360,25,12,0);
      rotateY(-turn/3+20);
      TorusEmoji(leaf[2],screen,screen/2,360,50,10,0);
    pop();
  pop();
...
function TorusCube(ObjModel,r0,r1,thetaMax,phiMin,phiMax,cubeNum,size,offset){
  for(let theta=0; theta<thetaMax; theta+=cubeNum){
    for(let phi=phiMin; phi<phiMax; phi+=cubeNum){
      push();
      let bump = 1+0.3*sin(phi*5)*sin(theta*5);
...
function TorusSphere(ObjModel,r0,r1,thetaMax,phiMin,phiMax,cubeNum,size,offset){
  for(let theta=0; theta<thetaMax; theta+=cubeNum){
    for(let phi=phiMin; phi<phiMax; phi+=cubeNum){
      push();
      let bump = 1+0.2*sin(phi*8)*sin(theta*6);
      let x = (r0+r1*bump * cos(phi+turn+offset))*sin(theta);
      let y = r1*bump * sin(phi+turn+offset);
      let z = (r0+r1*bump * cos(phi+turn+offset))*cos(theta);
      translate(x,y,z);
